...
tf.app.flags.DEFINE_string(
    'ps_hosts', '172.17.0.7:2222', 'Comma-separated list of hostname:port pairs')
tf.app.flags.DEFINE_string('worker_hosts', '172.17.0.8:2222,172.17.0.9:2222',
                           'Comma-separated list of hostname:port pairs')
tf.app.flags.DEFINE_string('job_name', None, 'job name: worker or ps')
tf.app.flags.DEFINE_integer('task_index', None, 'Index of task within the job')


def tower_loss(scope, images, labels, tower_grads):
        ...
        for step in xrange(FLAGS.max_steps):
            ...
            if step % 10 == 0:
                ...
                format_str = ('%s: step %d, loss = %.2f (%.1f examples/sec; %.3f '
                              'sec/batch)')
                f = open('/data/out_%s%d' %
                         (FLAGS.job_name, FLAGS.task_index), 'w+')
                print >> f, format_str % (datetime.now(), step, loss_value,
